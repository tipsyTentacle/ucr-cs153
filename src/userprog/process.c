...
process_execute (const char *cmd_line) 
{
  ...
  //##Program name is the first token of cmd_line
  file_name = strtok_r (cmd_copy, ARGV_DELIMIT, &strtok_helper);
  strlcpy (thread_name, file_name, sizeof thread_name);
  struct exec_helper exec_data;
  init_exec_helper (&exec_data, cmd_copy, true);
  lock_acquire (exec_data.exec_lock);
  //##Change cmd_line in thread_create to thread_name
  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (thread_name, PRI_DEFAULT, start_process, &exec_data); //## remove cmd_copy, Add exec to the end of these params, a void is allowed. Look in thread_create, kf->aux is set to thread_create aux which would be exec. So make good use of exec helper!
  if (tid != TID_ERROR) //##Change to !=
  {  
    void* eip;
    void* esp;
    if (load (cmd_line, &eip, &esp))
    {
      struct process_thread_struct newChild;
      newChild.data = (struct thread*) (esp - 0x1000);
      list_push_back (&(thread_current () -> child_list), &newChild.elem);
    }
    else
    {
      lock_release(exec_data.exec_lock);
      return TID_ERROR;
    }
...
start_process (void *file_name_)
{
  ...
  bool success;
  struct kernel_thread_frame *kf = (struct kernel_thread_frame*) (thread_current () ->stack);
  struct exec_helper *exec_data = (struct exec_helper*) (kf->aux);
  lock_acquire (exec_data->exec_lock);
...
load (const char *cmd_line, void (**eip) (void), void **esp) 
{
  ...
  file = filesys_open (cmd_line);  //## Set the thread's bin file to this as well! It is super helpful to have each thread have a pointer to the file they are using for when you need to close it in process_exit
  if (file == NULL) 
  {
...
void init_exec_helper (struct exec_helper *exec, char* cmd_line, bool success)
{
  exec->cmd_line = palloc_get_page(0);
  strlcpy (exec->cmd_line, cmd_line, sizeof(exec->cmd_line));
  exec->success = success;
  lock_init (exec->exec_lock);
}